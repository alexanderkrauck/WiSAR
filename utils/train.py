...
def train_subarchitecture(
    ...
    lr_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.1, patience=2, cooldown=1, verbose=True
    )
...
def get_dataset(sub):
    if sub:
        return data.RandomSamplingGridCutoutDataset(
            ...
            crop_shape=(128, 128),
            resample_image_every_n_draws=50,
        )
    else:
        return data.MultiViewTemporalDataset(
            mode="validation",
            preprocess_image_options={
                "use_mask": True,
                "equalize_hist": False,
...
def train(
    pretrained_sub_arch: sub_architectures.AbstractTorchArchitecture,
    show_results=True,
    load_from_saved=False,
    save_inference_to_json=False,
    scoring=False,
    model=sub_architectures.ConvolutionalAutoencoderV2,
    ...
    train_loader = val_loader = torch.utils.data.DataLoader(
        dataset, batch_size=32, num_workers=0, shuffle=True
    )
    ...
    if pretrained_sub_arch is None:
        ...
        if show_results:
            output_images = evaluate_model(
                trained_model, trained_model, val_loader, device
            )  # Evaluate model (Get image results)
        ...
        if load_from_saved:
            state_dict = torch.load(
                "saved_models/ConvolutionalAutoencoderV2/no_histogram_128_trained",
                map_location=device,
            )
        ...
        approach = architectures.BasicAutoencoderAnomalyDetectionV1(
            autoencoder, device=device, image_sizes=(1024, 1024)
        )  # Load in autoencoder
...
def HyperparameterSearch():
    ...
    param_grid = {
        "model_types": [sub_architectures.ConvolutionalAutoencoderV2],
    ...
    for item in list(ParameterGrid(param_grid)):
        trained_model_sub = train(
            pretrained_sub_arch=None,
            show_results=False,
        ...
        score = train(
            pretrained_sub_arch=trained_model_sub,
            show_results=False,
            save_inference_to_json=True,
            load_from_saved=False,
            scoring=True,
        )
        scores.append(
